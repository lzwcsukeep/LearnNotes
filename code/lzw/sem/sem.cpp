...
void sem_lock(int sem_set_id)
{
    ...
    // 下面三条语句都是描述要操作哪个信号量，以及怎么操作信号量。
    sem_op.sem_num = 0;  // 要操作的信号量在信号量集中的序号
    sem_op.sem_op = -1;  // P 操作，先把信号量的值减一，减一之后如果信号量的值小于0，则阻塞当前进程。  
    sem_op.sem_flg = 0;
...
void sem_unlock(int sem_set_id)
{
    ...
    sem_op.sem_num = 0;
    // V 操作，先对信号量加一操作，如果加一之后信号量的值小于等于0,说明有进程在等待，唤醒该信号量下的某个等待进程
    sem_op.sem_op = 1; /* <-- Comment 3 */
    sem_op.sem_flg = 0;
    //操作信号量
    /**
     * semop 接受的参数就三个，
     * 一个是信号量集的标识符（注意不是具体的某个信号量，而是信号量的集合）
     * 一个是sembuf 类型的结构体指针（也可以是数组）。结构体内部会描述具体要操作的信号量在信号量集中的序号，和操作类型（P还是V操作）
     * 第三个是第二个参数传进来的sembuf结构体数组的元素个数
    */
    semop(sem_set_id, &sem_op, 1);
 ...
 *      * output:   none.
 *       */
void add_country(int sem_set_id, int *countries_num, struct country *countries,
...
{
    /*add_country() 函数里面用了信号量，有同步的作用。但是do_child 没有。因此可能do_child 执行了一部分被切换。*/
 ...
 *        */
void do_parent(int sem_set_id, int *countries_num, struct country *countries)
{
    int i, num_loops;
...
int main(int argc, char *argv[])
{
    ...
    // 获取一个信号量集，这个信号量集中只有一个信号量，返回信号量集的唯一标识符sem_set_id
    ...
    *countries_num = 0;
    ...
    case -1:
        ...
        exit(1);
        break;
    case 0: // 子进程代码，子进程不断的往共享内存里面写数据,add_country(...) ;
